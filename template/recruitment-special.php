<?php 
	/* Template Name: Recruitment Special*/
	$pageid = get_the_ID();
	get_header();
	the_post();
?>
									...
									<div class="row">
										<div class="form-select form-group col-md-6">
											...
											<select class="form-control" name="phong_ban" id="department" autocomplete="off">
												...
												<?php 
													$offterms = get_terms('phong_ban_tuyen_dung_dac_biet',array('hide_empty' => false));
													if($offterms){
										...
										<div class="form-select form-group col-md-6">
											...
											<select class="form-control" name="khu_vuc" id="area" autocomplete="off">
												<option value="Tất cả">Tất cả</option>
												<?php 
													$areaterms = get_terms('khu_vuc_tuyen_dung_dac_biet',array('hide_empty' => false));
													if($areaterms){
					...
					<div class="list-recruitment">
						<?php
							...
							if(isset($_POST['khu_vuc']) && $_POST['khu_vuc'] != '') {
								$args['tax_query'][] = array(
									'taxonomy'	=> 'khu_vuc_tuyen_dung_dac_biet',
									'field'	=> 'id',
									'terms'	=> $_POST['khu_vuc']
								);
							}
							 ...
							 $postid = $post->ID;
							 $of_terms = wp_get_post_terms($postid,'phong_ban_tuyen_dung_dac_biet');
		                     $ad_terms = wp_get_post_terms($postid,'khu_vuc_tuyen_dung_dac_biet');
